...
app.use(
  cors({
    origin: [
      `http://localhost:5173`,
      `https://job-portal-p3.web.app`,
      `https://job-portal-p3.firebaseapp.com`,
    ],
    credentials: true,
  })
);
...
async function run() {
  try {
    ...
    app.post("/jwt", (req, res) => {
      const user = req.body; // atai amader payload
    ...
    app.get("/jobs", async (req, res) => {
      ...
      if (sort === "true") {
        sortQuery = { "salaryRange.min": -1 }; // sorting in descending order
      }
